...
(function($) {
    ...
    Bots.find = function(name) {
        ...
        assert(false, "No encontre un bot llamado: " + name);
        return false;
    }
    ...
    function assert(value, fmsg, tmsg) {
        if (!value) {
            if (fmsg === undefined) fmsg = "Assert fallido.";
            log(fmsg, true);
    ...
    function getSynonymList(original, list) {
        assert(list !== undefined, "No hay suficientes sinonimos '" + original + "'");
    ...
    function getByDelimiter(text, start, end) {
        ...
        assert(pieces.length > 0, "No suficientes piezas de texto " + text);
